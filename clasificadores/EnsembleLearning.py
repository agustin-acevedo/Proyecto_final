...
# Inicializar los clasificadores individuales
...
# Entrenar los clasificadores individuales
...
# Predecir con los clasificadores individuales
...
# Combinar las predicciones usando votación simple 
hybrid_pred = (rf_pred + gb_pred) // 2


#--------------------------------------------------------------------------
#OBTENCIÓN DE MÉTRICAS DE RENDIMIENTO
print ("OBTENCIÓN DE MÉTRICAS DE RENDIMIENTO")
p, recall, fscore, support = precision_recall_fscore_support(y_test,hybrid_pred, average='micro')
bacc = balanced_accuracy_score(hybrid_pred, y_test)

y_test = le.inverse_transform(y_test.astype(int))
hybrid_pred = le.inverse_transform(hybrid_pred.astype(int))
v_p, v_recall, v_fscore, v_support = precision_recall_fscore_support(y_test,hybrid_pred, average=None, labels = le.classes_)

cnf_matrix = confusion_matrix(y_test,hybrid_pred)
