...
class LazyBayesianRule:
    ...
    def predict(self, X):
        ...
        for x in X:
            ...
            X_neighbors = self.X_train[indices[0]]
            y_neighbors = self.y_train[indices[0]]
            
            # Entrenar un Naive Bayes localmente con los vecinos más cercanos
            self.gnb.fit(X_neighbors, y_neighbors)
            
            # Predecir la clase para la instancia actual
            prediction = self.gnb.predict([x])
...
# Entrenar el clasificador LBR
lbr = LazyBayesianRule(k=5)
