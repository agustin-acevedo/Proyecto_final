...
p, recall, fscore, support = precision_recall_fscore_support(y_test,bagging_pred, average='micro')
bacc = balanced_accuracy_score(bagging_pred, y_test)

y_test = le.inverse_transform(y_test.astype(int))
bagging_pred = le.inverse_transform(bagging_pred.astype(int))
v_p, v_recall, v_fscore, v_support = precision_recall_fscore_support(y_test,bagging_pred, average=None, labels = le.classes_)

cnf_matrix = confusion_matrix(y_test,bagging_pred)
